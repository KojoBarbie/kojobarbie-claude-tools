...
5. **人間の接点は GitHub に集約、通知は持たない** — 判断待ちは `data/pending.json` に、
   起きたことは `data/events.jsonl` に落とすところまでが責務で、誰にどう知らせるかは利用者が決める
   （[events.md](events.md)）。見落としてもサイクルは止まらない（デフォルト動作が決まっている）。
...
- PRD の PR に同じブランチで含める → **人間は PR 1本で「ジョブ分析・KPI 数値・見た目」を
  まとめてレビュー**できる。起票イベントに showcase の URL を添える
...
- **入口を GitHub issue にした理由**: App Factory の人間判断はすべて issue/PR に集約されている
  （feature-hunt の `go`／store-release の `approved`／PRD の マージ・クローズ）。
  通知経路を入力にも使うと経路が2系統になるので、**操作は必ず GitHub 側**に置く。
  受け手（Slack でもダッシュボードでも）は表示と導線だけを担当し、パイプラインの入力にはしない
...
- 満たさない場合は PR を open のまま残し、`pending.json` と週報で人間に回す
...
- **人間との修正のやり取りは GitHub 上で完結**: 実装前なら issue 本文の編集/コメント、実装後なら
  ...
  **同一 GitHub 名義のため bot の返信は人間に通知されない** → 対応した PR は factory-build が
  `kind=comment_answered` / `severity=action` のイベントを出す。今すぐ拾わせたいときは「ファクトリー回して」で手動起動可
...
- スクリーンショット: シミュレータ撮影の自動化は初期スコープ外。`docs/store-assets/` に
  置かれたものを使い、無ければ `human_task` イベントで1回だけ人間に依頼（これだけは人手）
...
- **提出は人間の承認ゲート付き（オプトイン）**。承認の受け口は GitHub issue:
  リリースごとに `release-train` ラベルの issue を起票し、`release_prepared` イベントに
  「内容を確認して承認（`approved` ラベル or 👍）したら提出します。止める/延期は `hold` or close」と書く。
...
- **初回提出は人間併走**: ASC API での提出が初めてのアプリは、承認済みでも提出直前でもう一度止めて
  `human_task` イベントで案内（App Privacy・輸出コンプラ等の初回特有の穴を確認。2回目以降は承認だけで全自動）
- 審査中アプリのステータスを毎回追跡し、リジェクトされたら内容を要約して週報 + `review_rejected` イベント
...
- **週報を1本書き出す**（`data/reports/`）: 今週生まれた/進んだ/リリースされたもの、各アプリの KPI 達成状況と傾向、
...
- すべてのジョブはジョブ実行環境（`$APP_FACTORY_HOME`）の共通規約に従う: config.env・`.env` 読込 →
  `claude -p --permission-mode bypassPermissions` → ログ → 失敗時 `job_failed` イベント。
