...
6. **安全レールで暴走を防ぐ** — auto-merge には機械的な条件、ストア提出には拒否権ウィンドウ、
   失敗の繰り返しにはブロックラベル。
...
| `review` | MVP完了・リリース準備〜Apple審査中 | store-release がメタデータ生成→veto付き提出→審査追跡 |
...
人が読む通知（veto 確認・週報）だけが日中に届くようにする。
...
| **毎日 6:00** | **store-release**（bash 事前判定で用がある日だけ起動。列車起票→veto→提出→審査追跡） | ★新規 |
...
- ASC API でバージョン作成→メタデータ投入→ビルド（タグ→TestFlight ワークフロー産）を紐付け
- **提出は24時間の拒否権ウィンドウ付き**。実現方法は GitHub issue（Slack の webhook は返信を
  読めないため）: リリースごとに `release-train` ラベルの issue を起票し、Slack には
  「24時間後に自動提出。止めるならこの issue に `hold` ラベル or close」と通知。
  翌日以降の実行時に issue が open のまま `hold` 無しなら提出する。
  この issue がリリース列車の状態機械（チェックリスト・審査状況コメント）も兼ねる
- **初回提出は人間併走**: ASC API での提出が初めてのアプリは提出直前で止めて Slack で案内
  （2回目以降は全自動）
...
- **週報を Slack に1通**: 今週生まれた/進んだ/リリースされたもの、各アプリの KPI 達成状況と傾向、
  ...
  **人間の要アクション一覧**（未マージ PRD PR / 未承認機能提案 / veto 中のリリース /
...
| 水〜木 | store-release の veto ウィンドウ確認（デフォルトは提出続行） | 提出される（それが既定の意図） |
