...
- **対話モード（既定）**: ステップ 2 でプランモードに入り、人間の承認を得てから実装する。
- **無人モード**: factory-build 等の自動ジョブ（launchd / `claude -p`）から呼ばれた場合、
  または「無人モードで」「承認なしで進めて」と明示された場合。
  **EnterPlanMode / ExitPlanMode は使わない**（headless では承認者がおらず、そこで停止してしまう）。
  ただし**計画を立ててから実装する、という順序は変えない**: ステップ 2 と同じ内容の計画を文章で作成し、
  実装開始**前に** issue へ `🤖 実装計画:` で始まるコメントとして投稿し、PR 本文の「実装計画」
  セクションにも残す。承認ゲートだけを外し、人間が事後に計画へ遡れる形を保つ。
  計画を書かずにコードを書き始めることは無人モードでも禁止。

## 前提と中断点

- **唯一の中断点はステップ 2（計画承認）**。ここ以外では原則ユーザーに確認を求めず自走する。
  無人モードでは中断点ゼロ（計画は承認なしで記録のみ）。
...
> **無人モードではこのステップのツールを使わない。** 下記「計画に必ず含めるもの」を満たす計画を
> 文章で作成し、issue に `🤖 実装計画:` コメントとして投稿してからステップ 3 へ進む（承認は求めない）。
...
- **アーキテクチャ・テスト戦略との整合**: リポジトリに `docs/architecture.md` / `docs/test-strategy.md`
  があれば計画をそれに整合させる（レイヤ配置・禁止事項・テストコマンド）。逸脱する場合は計画に理由を明記し、
  文書側の更新も同じ PR に含める
...
**PR 本文は「判断書」として書く。規約は `references/judge-block.md` を読んで従うこと。**

読者は「マージしていいか決める人」1人だけで、その人が知りたいのは
「自分は何をすればいいか」「どのくらいリスクがあるか」「放置したらどうなるか」の3つしかない。
説明を消すのではなく、**この3つを先頭に置き、従来の説明は `<details>` に畳む**（読む順序を変える）。
...
**<動詞で始まる1文。何をして、何がどうなっていれば OK か>**
それ以外は読まなくて大丈夫です。

## 何が変わるか

<2〜4行。ユーザーから見て何が変わるかを先に。見た目や操作が変わらないなら明記する>

| | |
|---|---|
| 種類 | 動かす（約5分） |
| リスク | 低 — UI のみ。課金 / 権限 / データ移行には触れていません |
| 放置したら | <expires> に自動マージします |
...
守るべき点だけ再掲（詳細は `references/judge-block.md`）:

- **`ask`（太字1文）はコードを読ませない**。「この実装で良いか見てください」は禁止。
  人間に頼んでよいのは機械が原理的に確認できないこと（実機の挙動・見た目・事業判断）だけ
- **`## 何が変わるか` を必ず書く**（2〜4行）。ask は「してほしいこと」であって
  「何を変えたか」ではない。両方ないと通す判断ができない。
  見た目や操作が変わらないなら「変わりません」と明記する
- `type` は `look`(スクショを見るだけ) / `run`(実機で動かす) / `decide`(方針を決める) の3つ。
  外部作業（Firebase 権限・ASC 設定等）が必要なら **PR ではなく `human-task` issue に切り出す**
- `risk` はセンシティブ領域（課金 / 権限 / データ移行 / CI 設定 / 署名）に触れていれば `high`。
  この値で時効の既定動作が決まるので、迷ったら重い方に倒す
- `expires` は作成日 + 14日。`shots` は撮れなかったら行ごと省略する
- **`Closes #<N>` は `</details>` の外**に置く（親の子を実装した場合も、その子番号を Closes する）
...
1. 画面を撮る（XCUITest の巡回テストがあればそれ、無ければ起動直後の1枚）。
   **保存先は `$APP_FACTORY_HOME/screenshots/<アプリ名>/<PR番号>/`**（リポジトリにはコミットしない）。
...
4. **判断ブロックを撮った結果に合わせて更新する**（PR 番号は作成後にしか決まらないのでここで確定させる）:
   ...
   - 撮れていて見た目の確認だけを頼むなら `type: look` / `cost: 0.2` に落とす
     （スクショがある PR を「実機で動かして」と頼むのは判断者の時間の浪費）

   ```bash
   gh pr view <pr> --json body -q .body > /tmp/body.md   # 編集して
