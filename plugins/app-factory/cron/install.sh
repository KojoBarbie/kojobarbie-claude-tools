...
#
# やること:
#   1. run スクリプト4本を claude-cron/scripts/ にコピー（既存ファイルは .bak 退避）
#   2. launchd plist 4本を「このスクリプトが $HOME から生成して」~/Library/LaunchAgents/ に配備・load
...
CRON_DIR="${CRON_DIR:-$HOME/dev/others/claude-cron}"
AGENTS_DIR="$HOME/Library/LaunchAgents"

SCRIPTS=(run_portfolio_review.sh run_factory_build.sh run_factory_dispatch.sh run_store_release.sh)
...
if [ ! -d "$CRON_DIR" ]; then
  echo "ERROR: claude-cron が見つかりません: $CRON_DIR （CRON_DIR 環境変数で変更可）"
  exit 1
fi
...
${cal}
    ...
    <dict>
        <key>PATH</key>
        <string>${HOME}/.nodebrew/current/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin</string>
        <key>HOME</key>
        <string>${HOME}</string>
    </dict>
</dict>
...
1. 初回ブートストラップ（portfolio.yml と割当表を作る）:
     cd ~/dev/others/claude-cron && claude "app-factory:portfolio-review を実行して"
...
3. factory-build の auto-merge は初期状態で OFF（PR 作成まで）。
   1〜2週様子を見て問題なければ ON にする:
     touch ~/dev/others/claude-cron/data/factory_automerge_enabled
