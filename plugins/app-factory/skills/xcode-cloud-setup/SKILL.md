...
App Store Connect API（`~/dev/others/claude-cron/.env` の `APP_STORE_*` キー）でXcode Cloud関連を自動化する。
...
| サブコマンド | 用途 |
|---|---|
| `register-bundle-id <identifier> <name>` | Bundle ID登録（例: `register-bundle-id com.kojobarbie.hirune Hirune`）。登録済みなら成功扱い |
...
スクリプトは ciProduct / scmRepository / 最新のXcode・macOSバージョンID（"Latest Release"優先＝自動追従）を自動解決する。オプション: `--scheme`（省略時はAppName）、`--project`（省略時は `{AppName}.xcodeproj`）、`--branch`（PRの宛先＝リポジトリのデフォルトブランチ。既定 `main`。**実行前に `gh repo view KojoBarbie/{AppName} --json defaultBranchRef` で必ず確認して合わせる**）。

## 自動検知の仕組み（前提）

通常はこのスキルを手で呼ぶ必要はない。`app-kickoff` がアプリ名を `~/dev/others/claude-cron/.data/pending_xcode_cloud.txt` に登録し、承認チェックジョブ（`run_prd_approval_check.sh`、毎日9時/21時）が `check-onboarded` でポーリング → 検知したら `create-workflows` 実行 → Slack通知、まで全自動で行う。3日以上未完了のアプリには3日おきに手動タスクのチェックリストをSlackに再送する。このスキルを手動で使うのは、自動作成が失敗したときのリカバリーや、既存アプリへの追加設定のとき。
