...
5. **要アクション（人間向け）**: 下記の各項目を **1件ずつ明細で** 出す（「◯件あります」で終わらせない）。
   ...
   - 各明細に必ず付ける4点:
     1. **タイトルと直リンク**（issue / PR の URL。「どれを見るか」が一発で分かるように）
     2. **具体的な手順**（例:「👍 か `go` ラベルで承認」「release-train issue を確認して `approved` ラベル or 👍」
        「この情報をください: 〜」「ASC でアプリ作成 + Xcode Cloud オンボーディング」）。
        承認系は**何を承認するのか**（提出してよいか / 機能をやってよいか等）を明記する
     3. **人間に渡してほしい情報がある場合はそれを具体的に**（needs-clarification は issue の質問文を要約して載せる）
     4. **放置した場合のデフォルト動作**（例: 提出承認待ち→「提出されないまま待機」、機能提案→「6件超で新規提案が自動休止」）
   - release-train の提出承認待ちは特に、対象バージョンとその根拠（マイナー↑/パッチ↑）・確認点
     （メタデータ/スクショ/著作権/カテゴリ）まで書く（store-release の起票 issue にある内容を転記）
