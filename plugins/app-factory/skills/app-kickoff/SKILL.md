...
- 内容は `app-design-craft` スキルの原則（抑制・階層・余白・言葉・一貫性・仕上げ）に沿って: 世界観（形容詞3つ）/ ライト・ダーク両パレット（HEX）/ タイポ指定 / 唯一の強調装置 / 避けたい表現・禁止語 / ボイス / 主要2〜3画面のレイアウト方針
...
PRDのコア機能を分解し、下記の**必須issueセット**と合わせて **6〜9件のissue** を登録する。各issueは `ship-issue` スキルでそのまま着手できる粒度にする:

- タイトル: 実装単位で具体的に。実装順が分かるよう先頭に `[1]` `[2]` を付ける
- 本文: ユーザーストーリー＋受け入れ条件（チェックボックス）＋PRD該当セクションへの言及
- **順序**: コア機能（1〜3件）→ 計測 → オンボーディング → ペイウォール → 設定画面 → （広告実装※PRDが広告収益化の場合）→ 最後に「App Store提出準備」

**必須issue（どのアプリでも必ず作る。コア機能以外はこの5本）**:
