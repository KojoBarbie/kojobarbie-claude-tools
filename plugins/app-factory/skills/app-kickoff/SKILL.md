...
- **冒頭に「方向性」を1行で明記する。** `app-design-craft` の⓪の8方向
  （`calm/serene` `playful/energetic` `premium/refined` `utilitarian/focused`
  `editorial/magazine` `brutalist/raw` `warm/organic` `cold/technical`）のどれかを名指しし、
  選ばれたトンマナ案がどの方向なのかを書く。**形容詞3つだけで済ませない**
  （形容詞は方向の言い換えになりやすく、「静か・温かい・潔い」が全アプリで再生産される）
- **書く前に重複を確認する。** `$PRD_VAULT_DIR/portfolio.yml` の新しい順に2本のアプリの
  `docs/design-concept.html` を見て、その方向性と今回が同じなら
  **完了イベントの `title` に警告を1行入れる**（「直近の {App} と同じ {方向} です」）。キックオフは止めない — トンマナ案は PRD の PR で
  ユーザーが既に承認しているので、ここで作り直さない。ただし黙って通すと同質化が進む
- 内容は `app-design-craft` スキルの9レンズに沿って: 世界観（形容詞3つ）/ パレット（HEX）/ タイポ指定 / 唯一の強調装置 / 避けたい表現・禁止語 / ボイス / 主要2〜3画面のレイアウト方針
  （**外観は単一。ライト/ダークの2系統は作らない。その1つが暗い外観でも構わない**）
