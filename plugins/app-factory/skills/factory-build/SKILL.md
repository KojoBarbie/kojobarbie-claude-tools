...
7. **PR 滞留ガード: open PR が10本以上のリポジトリには新規 PR を出さない**。
   対象リポジトリごとに open PR 数を数え、**10以上なら そのリポジトリからは issue を選定しない**
   （＝新規 PR を作らない）。
   ```bash
   gh pr list -R <owner/repo> --state open --limit 100 --json number -q 'length'
   ```
   ただし**ステップ0の作業（回収マージ・人間コメントへの対応）は滞留リポジトリでも通常どおり行う**
   — 滞留を減らす方向の作業なので止める理由がない。マージが進んで10本を切れば、次回から自動的に
   新規着手が再開する。狙い: 修正 PR が15〜16本と積み上がって人間がレビューしきれなくなるのを防ぐ
   （生産量ではなく**人間のレビュー帯域**がボトルネックなので、そこに合わせて流入を絞る）
...
**まず対象リポジトリを絞る（安全レール7 の滞留ガード）**。ステップ0 の回収マージが終わった**後**の
open PR 数を数え、**10本以上のリポジトリは本ステップの対象から外す**（issue を1件も選ばない）:

```bash
gh pr list -R <owner/repo> --state open --limit 100 --json number -q 'length'
```

外したリポジトリはステップ4の報告に `アプリ名 — PR滞留のため新規着手を見送り（open N本）` と1行残す。
...
- **PR 滞留でスキップしたアプリ**: `アプリ名 — PR滞留のため新規着手を見送り（open N本）→ <PR一覧URL>`
  （`https://github.com/<owner>/<repo>/pulls`）。「レビュー/マージを進めると次回から再開します」を添える
...
- **対象 issue ゼロ**: 正常。ステップ4 の「着手可能な issue なし（異常なし）」1行だけを出して終了。
  ただし**理由が PR 滞留のとき（全アプリが open PR 10本以上）は「異常なし」で済ませない** —
  `🏭 factory-build <時刻> — 全アプリが PR 滞留のため新規着手なし` に続けて、アプリ名・open 本数・
  PR一覧URL を各1行で出す（人間がレビューを進めないと止まったままなので、行動を促す必要がある）
...
- **手動実行**: 件数上限をユーザーが指定できる（「5件やって」）。auto-merge 条件は変えない。
  滞留ガード（安全レール7）も既定では効かせるが、ユーザーが明示的に解除を指示した場合
  （「PR 溜まってても回して」等）はそのアプリに限り無視してよい。
...
- **PR 滞留ガード（open 10本で新規停止）**は、生産側ではなく**人間のレビュー帯域**がこのループの
  真のボトルネックだから。修正 PR が15〜16本溜まると人間が処理を諦め、PR の山ごと死ぬ。
  上限を設けると「作る」より「捌く」が優先され、滞留は回収マージと人間コメント対応で自然に解消する
  （ステップ0 は滞留中も止めないので、詰まったまま固定されることはない）。
  10本という数字は「一気に見返せる上限」の経験値で、キツすぎたら緩める前提の可変パラメータ
