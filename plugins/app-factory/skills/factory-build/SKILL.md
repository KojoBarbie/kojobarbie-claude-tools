...
description: >
  ...
  1回の実行で最大2 issue（同一アプリからは1 issue まで）。auto-merge は機械的な安全条件を全て満たしたときだけ。
---
...
1. **1回の実行で最大2 issue、かつ同一アプリからは最大1 issue**（＝2件やるなら必ず別アプリ）。
   それ以上は次回（1日2回動く）に回す。**1アプリ＝1回のビルドで1変更**に保つのが狙い
...
## ステップ 1: issue の選定（最大2件・**同一アプリからは1件まで**）
...
3. **2件目は、1件目とは別アプリの候補から**、同じ優先度順で最上位のものを選ぶ
   （＝**同一アプリからは1回の実行で1 issue まで**。1件目と同じアプリの残り候補は全部スキップして次回へ）
4. 別アプリの着手可能な候補が無ければ、2件目は取らずに1件で終える
   （無理に同一アプリから2件目を取らない — antiscroll のような在庫の多いアプリに2枠を占有させない）
