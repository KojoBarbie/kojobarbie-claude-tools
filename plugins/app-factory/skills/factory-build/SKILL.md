...
0. **`priority-high` ラベルの issue（全アプリ横断で最優先）— 差し込み枠**。
   「他の何より先にこれを通したい」を人間が指定するためのレーン。用途は限定しない
   （緊急の UI/UX 改善、期間を区切って一気に通したい横断作業、リリース前の詰め など）。
   同一アプリ内では番号昇順。**このティアが空になったら通常の優先度に戻る**。
   **このラベルを factory-build 自身が付けてはいけない。** 付けるのは人間か、
   人間から明示的に指示されたエージェントだけ。自分で付けられると差し込み枠が意味を失う
...
5. **上のどれにも当たらない open issue**（stage 優先度順 → 番号昇順）。
   ラベルの付いていない改善 issue が**永久に拾われず在庫として腐る**のを防ぐための受け皿。
   ここに落ちてくる issue は起票の意図がラベルから読めないので、
   **受け入れ条件が本文に書かれていないものは着手せず `needs-clarification` を付けて次へ**
